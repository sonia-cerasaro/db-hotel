...
var app = new Vue ({
  ...
  data: {
    rooms: [],
    room: {}          //se metto NULL non si vedra' neppure l'estetica html di v-if
  },
  mounted() {
  ...
  methods: {
    getById: function(id) {
      ...
      .then((response) => {                                           //al click su di una stanza mi resituira' i suoi valori (quindi visualizzera' le info contenute al suo interno)
        this.room = response.data.response[0];                        //response [0] e' collegato al div con V-if che solo al click mi mostrera' i dati altrimenti nulla [indice 0]
      });
    }
  }
